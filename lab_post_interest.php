<?php
	...
	function addToLInterestDatabase($title_2, $body_2, $article_id2){
		global $connect;
		global $unique_id;
		global $username;
		...
		$insert_query="insert into lab_post_interest values (NULL, $article_id2, $unique_id, '$username', '$title_2', '$body_2');";
		$result=mysqli_query($connect, $insert_query);
		if($result){

			//Update Interaction_status Database
			$department_query = "select department from lab_recruiting_post where article_id = $article_id2";
			...
			$update_query = "update interaction_status set student_to_lab_interest = student_to_lab_interest + 1 where department_id = $interest_department";
		...
		<!-- Wrapper -->
			<div id="wrapper">
				...
				<!-- Main -->
					<div id="main">
						<div class="inner">
							...
							<!-- Content -->
								<section>
									<header class="main">
										<h1>Contact to Lab</h1>
									</header>

									<form method="post" action="lab_post_interest.php">
										<?php 
					...
					<?php
						if($title !="" and $body !=""){
							$isSaved = addToLInterestDatabase($title, $body, $article_id);
							if($isSaved){
