<?php 
	session_start();

	global $id;
	global $password;
	global $connect;

	$id = $_POST["id"];
	$password = $_POST["password"];

	$connect=mysqli_connect("localhost","root","rhfkffk23","klmu");

	if($connect == false) {
		echo "connection failed! <br>";
	}
	function authentication($id_2, $password_2){
		global $connect;
		$checkid_query_student="select id from student_account where id = '$id_2'";
		$checkid_query_lab="select id from lab_account where id = '$id_2'";

		$result_id_student=mysqli_query($connect, $checkid_query_student);
		$result_id_lab=mysqli_query($connect, $checkid_query_lab);
		if($row = mysqli_fetch_array($result_id_student)){ // If ID exists in database
			$checkpw_query="select password from student_account where password = '$password_2'";
			$result_pw=mysqli_query($connect, $checkpw_query);
			if($result_pw){ 	//If Password exists in database
				if($row = mysqli_fetch_array($result_pw)){	//Login Success
					
					//Initialize session variable 'username'
					$checkname_query = "select name from student_account where id = '$id_2'";
					$result_name=mysqli_query($connect, $checkname_query);
					if($result_name){
						$row = mysqli_fetch_array($result_name);
						$_SESSION['username'] = $row[0];
					}
					else{
						echo "checkname query failed!<br>";
					}

					//Initialize session variable 'type'
					$checktype_query = "select type from student_account where id = '$id_2'";
					$result_type=mysqli_query($connect, $checktype_query);
					if($result_type){
						$row = mysqli_fetch_array($result_type);
						$_SESSION['type'] = $row[0];
					}

					//Initialize session variable 'user_id'
					$checktype_query = "select unique_id from student_account where id = '$id_2'";
					$result_type=mysqli_query($connect, $checktype_query);
					if($result_type){
						$row = mysqli_fetch_array($result_type);
						$_SESSION['unique_id'] = $row[0];
					}

					//Initialize session variable 'department'
					$checkdepartment_query = "select department from student_account where id = '$id_2'";
					$result_department = mysqli_query($connect, $checkdepartment_query);
					if($result_department){
						$row = mysqli_fetch_array($result_department);
						$_SESSION['department'] = $row[0];
					}

					//Initialize session variable 'islogin'
					$_SESSION['islogin'] = 'logged_in';
					return true;
				}
				else {
					echo "Wrong Password<br>";
					return false;
				}
			}
			else{
				echo "checkpw_query failed!<br>";
				return false;
			}
		}
		else if($row = mysqli_fetch_array($result_id_lab)){
			$checkpw_query="select password from lab_account where password = '$password_2'";
			$result_pw=mysqli_query($connect, $checkpw_query);
			if($result_pw){ 	//If Password exists in database
				if($row = mysqli_fetch_array($result_pw)){	//Login Success
					
					//Initialize session variable 'username'
					$checkname_query = "select name from lab_account where id = '$id_2'";
					$result_name=mysqli_query($connect, $checkname_query);
					if($result_name){
						$row = mysqli_fetch_array($result_name);
						$_SESSION['username'] = $row[0];
					}
					else{
						echo "checkname query failed!<br>";
					}

					//Initialize session variable 'type'
					$checktype_query = "select type from lab_account where id = '$id_2'";
					$result_type=mysqli_query($connect, $checktype_query);
					if($result_type){
						$row = mysqli_fetch_array($result_type);
						$_SESSION['type'] = $row[0];
					}

					//Initialize session variable 'user_id'
					$checktype_query = "select unique_id from lab_account where id = '$id_2'";
					$result_type=mysqli_query($connect, $checktype_query);
					if($result_type){
						$row = mysqli_fetch_array($result_type);
						$_SESSION['unique_id'] = $row[0];
					}

					//Initialize session variable 'department'
					$checkdepartment_query = "select department from lab_account where id = '$id_2'";
					$result_department = mysqli_query($connect, $checkdepartment_query);
					if($result_department){
						$row = mysqli_fetch_array($result_department);
						$_SESSION['department'] = $row[0];
					}

					//Initialize session variable 'islogin'
					$_SESSION['islogin'] = 'logged_in';
					return true;
				}
				else {
					echo "Wrong Password<br>";
					return false;
				}
			}
			else{
				echo "checkpw_query failed!<br>";
				return false;
			}
		}
		else {
			echo "Wrong ID<br>";
			return false;
		}
		
		
	}
	
	$visit_query = "select count from visit_count";
	$visitor_count;
	$result_visit=mysqli_query($connect, $visit_query);
	if($result_visit){
		if($row = mysqli_fetch_array($result_visit)){
			$visitor_count = $row[0]+1;
			$insert_query = "update visit_count set count = count + 1";
			$result_insert = mysqli_query($connect, $insert_query);

		}
	}
?>
<!DOCTYPE HTML>
<!--
	Editorial by HTML5 UP
	html5up.net | @ajlkn
	Free for personal and commercial use under the CCA 3.0 license (html5up.net/license)
-->
<html>
	<head>
		<title>KLMU</title>
		<meta charset="utf-8" />
		<meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=no" />
		<!--[if lte IE 8]><script src="assets/js/ie/html5shiv.js"></script><![endif]-->
		<link rel="stylesheet" href="assets/css/main.css" />
		<!--[if lte IE 9]><link rel="stylesheet" href="assets/css/ie9.css" /><![endif]-->
		<!--[if lte IE 8]><link rel="stylesheet" href="assets/css/ie8.css" /><![endif]-->
	</head>
	<body>

		<!-- Wrapper -->
			<div id="wrapper">

				<!-- Main -->
					<div id="main">
						<div class="inner">

							<!-- Header -->
								<header id="header">
									<a href="index.php" class="logo"><strong>KLMU</strong>  KAIST LAB MATCHER for Undergraduate</a>
									<a href="login.php" class="logo" align="right"><strong>LOGIN</strong></a>
								</header>

							<!-- Content -->
								<section align="">
									<header class="main">
										<h1>LOGIN</h1>
									</header>
									<!-- Elements -->
										<div class="row 200%">
											<div class="6u$ 12u$(medium)">
												<!-- Form -->
												
												<form method="post" action="login.php">
													
														<div class="6u 12u$(xsmall)">
															<input type="text" name="id" id="demo-name" value="" placeholder="ID" />
														</div>
														<br>
														<div class="6u$ 12u$(xsmall)">
															<input type="password" name="password" id="demo-email" value="" placeholder="Password" />
														</div>
														<br>
														<!-- Break -->
														<div class="12u$">
															<ul class="actions">
																<li><input type="submit" value="　　　　　　LOG IN　　　　　　　" class="special" /></li>
															</ul>
														</div>
														<div>
														　New to KLMU?&nbsp<a href="signup.php" class="logo">Create an account.</a>
														</div>
												</form>
											</div>
										</div>
								</section>

						</div>
					</div>
					<?php
						$b = $_POST['id'] ;
						$c =  $_POST['password'];
						
						if($_POST['id'] !="" and $_POST['password'] !=""){

							$auth = authentication($id, $password);
							if($auth){
								header("Location: index.php");
								exit();
							}	
							else {
								echo "<script>";
								echo 'alert("잘못된 회원정보입니다.")';
								echo "</script>";
							}
						}
					
					?>
				<!-- Sidebar -->
					<div id="sidebar">
						<div class="inner">

							
							<!-- Status -->
								<section id="status" class="alt">
									<header class="major">
										<h2>Status</h2>
									</header>
									<?php 
										if(!strcmp($islogin,'logged_in')){
											echo "<strong>Hello, $username!</strong><br>";
											if($type == "1"){
												echo "(Student User)<br>";
											}
											else if($type =="2"){
												echo "(Lab User)<br>";
											}

											
											if($type == "1"){
												$interaction_query = "select lab_to_student_interest from interaction_status where department_id = $department";
											}
											else if($type == "2"){
												$interaction_query = "select student_to_lab_interest from interaction_status where department_id = $department";
											}
											$result_interaction = mysqli_query($connect, $interaction_query);

											$row2 = mysqli_fetch_array($result_interaction);
											$interaction_count = $row2[0];


											if($department == "1" && $type == "1"){
												echo "Department : ISYSE<br>";
												echo "*현재까지 <strong style='color: #f56a6a'>$interaction_count</strong><strong style='color: #f56a6a'>번</strong> ISYSE 연구실에서 학생의 게시물에 컨택하였습니다.";
											}
											else if($department == "2" && $type == "1"){
												echo "Department : CS<br>";
												echo "*현재까지 <strong style='color: #f56a6a'>$interaction_count</strong><strong style='color: #f56a6a'>번</strong> CS 연구실에서 학생의 게시물에 컨택하였습니다.";
											}
											else if($department == "3" && $type == "1"){
												echo "Department : EE<br>";
												echo "*현재까지 <strong style='color: #f56a6a'>$interaction_count</strong><strong style='color: #f56a6a'>번</strong> EE 연구실에서 학생의 게시물에 컨택하였습니다.";
											}
											else if($department == "1" && $type == "2"){
												echo "Department : ISYSE<br>";
												echo "*현재까지 <strong style='color: #f56a6a'>$interaction_count</strong><strong style='color: #f56a6a'>번</strong> ISYSE 학생이 연구실의 게시물에 컨택하였습니다.";
											}
											else if($department == "2" && $type == "2"){
												echo "Department : CS<br>";
												echo "*현재까지 <strong style='color: #f56a6a'>$interaction_count</strong><strong style='color: #f56a6a'>번</strong> CS 학생이 연구실의 게시물에 컨택하였습니다.";
											}
											else if($department == "3" && $type == "2"){
												echo "Department : EE<br>";
												echo "*현재까지 <strong style='color: #f56a6a'>$interaction_count</strong><strong style='color: #f56a6a'>번</strong> EE 학생이 연구실의 게시물에 컨택하였습니다.";
											}
										}
										else {
											echo '<strong>Could you Log-in first?</strong>';
										}
									?>
								</section>
							<!-- Menu -->
								<nav id="menu">
									<header class="major">
										<h2>Menu</h2>
									</header>
									<ul>
										<li><a href="index.php">Homepage</a></li>
										<li><a href="lab_information.php">Lab Information</a></li>
										<li><a href="lab_review.php">Lab Review</a></li>
										<li><a href="student_applying_board.php">Student Applying Board</a></li>
										<li><a href="lab_recruiting_board.php">Lab Recruiting Board</a></li>
										
									</ul>
									<br>
									<?php 
										echo "<text>사이트 방문자 수 : $visitor_count</text>"
									?>
								</nav>

							
						</div>
					</div>

			</div>

		<!-- Scripts -->
			<script src="assets/js/jquery.min.js"></script>
			<script src="assets/js/skel.min.js"></script>
			<script src="assets/js/util.js"></script>
			<!--[if lte IE 8]><script src="assets/js/ie/respond.min.js"></script><![endif]-->
			<script src="assets/js/main.js"></script>

	</body>
</html>