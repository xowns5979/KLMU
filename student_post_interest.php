<?php
	...
	function addToSInterestDatabase($title_2, $body_2, $article_id2){
		global $connect;
		global $unique_id;
		global $username;
		...
		$insert_query="insert into student_post_interest values (NULL, $article_id2, $unique_id, '$username', '$title_2', '$body_2');";
		$result=mysqli_query($connect, $insert_query);
		if($result){

			//Update Interaction_status Database
			$department_query = "select department from student_applying_post where article_id = $article_id2";
			$result_department = mysqli_query($connect, $department_query);

			$row2 = mysqli_fetch_array($result_department);
			$interest_department = $row2[0];

			$update_query = "update interaction_status set lab_to_student_interest = lab_to_student_interest + 1 where department_id = $interest_department";
		...
		<!-- Wrapper -->
			<div id="wrapper">
				...
				<!-- Main -->
					<div id="main">
						<div class="inner">
							...
							<!-- Content -->
								<section>
									<header class="main">
										<h1>Contact to Student</h1>
									</header>

									<form method="post" action="student_post_interest.php">
										<?php 
											echo "<input type='hidden' name='article_id' value=$article_id>";
										?>
										<div class="6u 12u$(xsmall)">
											<input type="text" width="100" name="title" id="demo-name" value="" placeholder="Title"/>
										</div>
										<br>
										<!-- Break -->
										<div class="12u$">
											<textarea name="body" id="demo-message" placeholder="Enter your message" rows="6"></textarea>
										</div>
										<br>
										<!-- Break -->
										<div class="12u$">
					...
					<?php
						if($title !="" and $body !=""){
							$isSaved = addToSInterestDatabase($title, $body, $article_id);
							if($isSaved){
								header("Location: student_applying_board.php");
								exit();
							}
							else {
								echo "<script>";
